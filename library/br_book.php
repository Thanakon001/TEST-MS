...
<body class="bg-info">
    ...
    <div class="d-flex align-items-center justify-content-center flex-grow-1 min-vw-100 vh-100">
        ...
        <div class="bg-white w-50 rounded-1 shadow-lg p-3 py-3 overflow-hidden" style="height: 70%;">
            ...
            <div class="container py-5">
                ...
                <div class="d-flex flex-column align-items-center gap-2">
                    ...
                    <form action="" method="post" class="col-8 d-flex align-items-center justify-content-center gap-2">
                        <span class="col-3 text-end">ผู้ที่ต้องการยืม : </span>
                        <input type="text" class="form-control w-50" name="m_user" placeholder="กรอกชื่อผู้ใช้">
                        <input type="submit" name="sub_member" class="btn btn-info text-white" value="ตกลง">
                    </form>

                    <?php
                    require_once 'conn.php';
                    session_start();
                    $_SESSION['member'] ? $_SESSION['member'] : [];
                    $_SESSION['book'] ? $_SESSION['book'] : [];


                    if (isset($_POST['sub_member'])) {
                        $user = $_POST['m_user'];

                        if (empty($_POST['m_user'])) {
                            echo "<script>Alert('กรุณากรอกชื่อผู้ใช้','error')</script>";
                            return;
                        }

                        $sql = "SELECT * FROM tb_member WHERE m_user = '$user'";
                        $result = $conn->query($sql);

                        if ($result->num_rows > 0) {
                            $data = $result->fetch_assoc();
                            $_SESSION['member']['m_user'] = $data['m_user'];
                            $_SESSION['member']['m_name'] = $data['m_name'];
                        } else {
                            $_SESSION['member'] = [];
                            echo '<script>Alert("ไม่พบข้อมูลสมาชิก","error")</script>';
                        }
                    }
                    ?>

                    <form action="" method="post" class="col-8 d-flex align-items-center justify-content-center gap-2">
                        <span class="col-3 text-end">รหัสหนังสือ : </span>
                        <input type="text" class="form-control w-50" name="b_id" placeholder="กรอกรหัสหนังสือ">
                    ...
                    if (isset($_POST['sub_book'])) {
                        ...
                        $sql = "SELECT * FROM tb_book WHERE b_id = '$book'";
                        $result = $conn->query($sql);

                        if ($result->num_rows > 0) {
                            $data = $result->fetch_assoc();
                            $_SESSION['book']['b_id'] = $data['b_id'];
                            $_SESSION['book']['b_name'] = $data['b_name'];
                        } else {
                            $_SESSION['book'] = [];
                            echo '<script>Alert("ไม่พบข้อมูลหนังสือ","error")</script>';
                        }
                    }
                    ?>
                </div>
                ...
                <div class="d-flex flex-column align-items-center gap-2 py-4">
                    <table class="table table-dark w-75">
                        <tr>
                            ...
                            <td><?= $_SESSION['member'] ? $_SESSION['member']['m_name'] : '' ?></td>
                        </tr>
                        <tr>
                            <td class="table-active text-end" style="width:40%">รหัสหนังสือ : </td>
                            <td><?= $_SESSION['book'] ? $_SESSION['book']['b_id'] : '' ?></td>
                        </tr>
                        <tr>
                            <td class="table-active text-end" style="width:40%">ชื่อหนังสือ : </td>
                            <td><?= $_SESSION['book'] ? $_SESSION['book']['b_name'] : '' ?></td>
                        </tr>
                        <tr>
                            <td colspan="2" class="table-active text-center">
                                ...
                                if (isset($_POST['sub_borrow'])) {
                                    ...
                                    if (empty($_SESSION['member']) && empty($_SESSION['member'])) {
                                        echo '<script>Alert("กรุณาเพิ่มข้อมูลชื่อผู้ใช้และหนังสือ","error")</script>';
                                        return;
                                    }
                                    
                                    $b_id = $_SESSION['book']['b_id'];
                                    $stmtCheck = "SELECT * FROM tb_borrow_book WHERE b_id = '$b_id' AND br_date_rt IS NULL";
                                    $resultCheck = $conn->query($stmtCheck);

                                    if ($resultCheck->num_rows > 0) {
                                        $text = "หน้งสือเล่มนี้ถูกยืมไปแล้ว";
                                        echo "<script>Alert('$text', 'error')</script>";
                                    } else {
                                        $date = new DateTime();
                                        $br_date_br = $date->format('y-m-d');
                                        $m_user = $_SESSION['member']['m_user'];
